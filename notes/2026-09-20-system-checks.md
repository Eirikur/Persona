...
Two independent checks, one generic way to run either from the chat UI.
...
| `persona_hub.py`            | one route, `POST /run_script/{name}`                        |
...
`4fda977` pipeline button.

**The live hub must be restarted** (`./persona_restart.sh`) before the Test
tab buttons work: `/run_script` is a new route. Everything was tested against
a throwaway second hub on port 8499, never the live one.
...
- The system persona (Cissy) speaking the summary *after* a run, e.g.
  "Recognition 0.9 s, inference 2.1, render 3.4". After, not during: speaking
  while the pipeline runs competes for the same CPU as the Chatterbox render
  and would skew the numbers being measured.
